...
onready var menus = [ $PauseMenu/MainMenu, $PauseMenu/GFXMenu, $PauseMenu/SFXMenu, ];
...
func wavify (item: RichTextLabel, amp: int = 25, freq: int = 1): item.bbcode_text = "[color=yellow][wave amp=" + str(amp) + " freq=" + str(freq) + "]" + str(item.text) + "[/wave][/color]";
...
func go_to_menu (menu: int):
	...
	if 0 > menu or menu_count <= menu: return;
...
func cam_lean_select (delta: int, label: RichTextLabel):
	...
	if 1 == delta: Settings.camera_lean = (1 + Settings.camera_lean) % 3;
	else: Settings.camera_lean = Settings.camera_lean - 1 if 0 < Settings.camera_lean else CameraLeanAmount.MAX;
...
func volume_select (delta: int, label: RichTextLabel):
	...
	match selected:
		1: 
			...
			label.text =("\nGAME VOLUME: " +                                        # title
						 ...
						 str(Settings.volume_game) + "  " +                         # convert value to string and concat
	...
	EventBus.emit_signal("volume_changed","game" if 1 == selected else "music" if 2 == selected else "sfx");
...
func prepare_labels ():
	...
	for sfx in range(1, labels[SUBMENU.SFX].size()):
		...
		match sfx:
			1:  label.text="\nGAME VOLUME: " + ("<" if 0 < Settings.volume_game else " ") + "  " + (" " if 10 > Settings.volume_game else "") + str(Settings.volume_game) + "  " + (">" if 10 > Settings.volume_game else " ") + "\n";
			2:  label.text="\nMUSIC VOLUME: " + ("<" if 0 < Settings.volume_music else " ") + "  " + (" " if 10 > Settings.volume_music else "") + str(Settings.volume_music) + "  " + (">" if 10 > Settings.volume_music else " ") + "\n";
			3:  label.text="\nSFX VOLUME: " + ("<" if 0 < Settings.volume_sfx else " ") + "  " + (" " if 10 > Settings.volume_sfx else "") + str(Settings.volume_sfx) + "  " + (">" if 10 > Settings.volume_sfx else " ") + "\n";
	pass;
